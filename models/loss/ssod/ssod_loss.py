...
class ComputeStudentMatchLoss():
        ...
        for t in targets:
            ...
            #伪标签低阈值和高阈值之间的，标记为不确定样本
                ...
                if self.pseudo_label_with_obj:
                    ...
                    if t[7] >= 0.99: #0.9 0.8
                        uncertain_obj_targets.append(np.concatenate((t[:6], t[7:8])))
                        #print(f'uc_obj:{len(uncertain_obj_targets)}')
                    #不确定样本里cls特别高的，送出来修cls loss
                    if t[8] >= 0.99: #0.9 0.8
                        uncertain_cls_targets.append(np.concatenate((t[:6], t[7:8])))
                        #print(f'uc_cls:{len(uncertain_cls_targets)}')
                else:
                    uncertain_targets.append(t[:7])
    ...
    def ota_loss(self, p, targets):
        ...
        if targets.shape[1] > 6:
            ...
            bs, as_, gjs, gis, reliable_targets, anchors, tscores = self.assigner(p, reliable_targets, with_pseudo_score = True)
            uc_bs, uc_as_, uc_gjs, uc_gis, uc_targets, uc_anchors, uc_tscores = self.assigner(p, uc_targets, with_pseudo_score = True)
